...
def mark_info(mark: MarkPayloadItem):
    return Texts.MySchool.MARK_INFO(
...
def marks_sorted_by_date_info(marks: Marks):
    ...
    return {
        date_str: Texts.MySchool.MARKS_FOR_DATE(DATE=date_str) + "\n\n".join([
            "\n".join(marks)
...
def marks_short_item(item: ShortSubjectPayloadItem, allow_goals: bool = False):
    ...
    return Texts.MySchool.SUBJECT_MARKS_INFO(
...
def marks_sorted_by_subject_info(marks_short: ShortSubjectMarks, goals: bool = False) -> dict[str, str]:
    return {
        (
            item
                .subject_name
                    .replace(Texts.MySchool.OBZ, Texts.MySchool.OBZ_SHORT)
                        .replace(Texts.MySchool.PHIZ_KULTURA, Texts.MySchool.PHIZ_KULTURA_SHORT)
        ): info
