...
async def enable_scheduler_cmd(message: Message, apis: APIs, user: User, bot: Bot):
    ...
    if message.chat.type == ChatType.PRIVATE:
        await message.answer(
            Texts.MySchool.SCHEDULER_UNAVAILABLE_IN_PRIVATE_CHATS
        )
        return
    elif (
        ...
        await message.answer(
            Texts.MySchool.YOU_MUST_BE_OWNER
        )
        return
    elif str(message.chat.id) in user.get("scheduler", {}):
        await message.answer(
            Texts.MySchool.SCHEDULER_ALREADY_ENABLED
        )
        return
    ...
    await message.answer(
        Texts.MySchool.SCHEDULER_ENABLED
    )
...
async def disable_scheduler_cmd(message: Message, user: User):
    ...
    if message.chat.type == ChatType.PRIVATE:
        ...
        await message.answer(
            Texts.MySchool.SCHEDULER_NOT_ENABLED
        )
        return
    ...
    await message.answer(
        Texts.MySchool.SCHEDULER_DISABLED
    )
